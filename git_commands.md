...
- ***`git add <filename1> <filename2>`*** : adds given untracked file to staging area for tracking. &nbsp we can add as many files here as we want.  to add all files at once ***`"git add ."`*** (not recommended, as it is not safe).
